...
// Extended types for Cerebras-specific streaming fields
interface CerebrasChunkDelta {
...
export const streamCerebras: StreamFunction<'cerebras'> = (
	...
	const stream = new AssistantMessageEventStream<'cerebras'>();

	(async () => {

		const startTimestamp = Date.now();
		const output: BaseAssistantEventMessage<'cerebras'> = {
			role: "assistant",
			api: model.api,
			model: model,
			id,
		...
		let finalResponse: ChatCompletion = getMockCerebrasMessage();
		...
		try {
			...
			for await (const chunk of cerebrasStream) {
				...
				const delta = choice.delta as typeof choice.delta & CerebrasChunkDelta;
				...
				if (chunk.usage) {
					const usage = chunk.usage as typeof chunk.usage & CerebrasUsage;
			...
			if (accumulatedReasoning) {
				(finalResponse.choices[0].message as any).reasoning = accumulatedReasoning;
			}

			stream.push({ type: "done", reason: output.stopReason, message: { ...output, timestamp: Date.now() } });

			const baseAssistantMessage: BaseAssistantMessage<'cerebras'> = {
				...output,
